...
class ODEFunc(nn.Module):
    def __init__(self,trunc):
        ...
        for m in self.lin.modules():
            if isinstance(m, nn.Linear):
                m.weight=nn.Parameter(torch.from_numpy(Linear(64)).float())
...
def get_batch(t,true_y):
    ...
    s = torch.from_numpy(np.random.choice(np.arange(length - args.batch_time, dtype=np.int64), args.batch_size, replace=False))
...
if __name__ == '__main__':
    ...
    ###########################################################################
    # Import KS data
    KS = scipy.io.loadmat('ks_L22_final.mat')
    a = KS['ut']
    a=a[:-1,:]
    [trunc,M]=a.shape
    print('Total Data: ',M)
    ...
    a=a.transpose()
    ts=KS['tt']
    ts=ts.squeeze() #dt=.25 time units

    # Convert to torch tensor
    true_y=torch.tensor(a[:args.data_size,np.newaxis,:trunc])
    true_y=true_y.type(torch.FloatTensor)
    t=torch.tensor(ts[:args.data_size])
    t=t.type(torch.FloatTensor)

    # Convert to torch tensor
    test_y=torch.tensor(a[args.data_size:,np.newaxis,:trunc])
    test_y=test_y.type(torch.FloatTensor)
    test_t=torch.tensor(ts[args.data_size:])
    ...
    plt.plot(np.arange(args.test_freq,args.niters+1,args.test_freq),np.asarray(test_err),'--')
